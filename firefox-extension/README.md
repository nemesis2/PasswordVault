...
| `vault-session.js` | The UI-free **session core** — the decrypted `SESSION`, the v6 unlock/decrypt loop, the hostname matcher, the clipboard auto-clear, the **passkey** create/get/precheck ceremonies, the authenticated POST + integrity-manifest **re-sign** (`_resignVault`), and the crypto self-test. **Shared byte-for-byte** with `chrome-extension/vault-session.js` (loaded by Firefox's background page and Chrome's offscreen document) — keep the two copies identical. |
| `background.js` | The Firefox **host**: the toolbar icon/badge, idle auto-lock, tab autofill, the passkey **approval window** plumbing, the popup **message handler**, and the thin `unlock`/`abortUnlock`/`lock`/`_progress` wrappers around `vault-session.js`. (Firefox holds the session in this persistent page directly; Chrome's equivalent host is split into a service worker + offscreen document.) |
