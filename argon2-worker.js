...
// { id, error:string } on failure. Inputs are NOT transferred, so the caller
// keeps its copies and can fall back to a main-thread derivation if needed.
self.onmessage = function (e) {
    var d = e.data;
    Promise.resolve()
        .then(function () { return globalThis.argon2idHash(d.password, d.salt, d.opts); })
        .then(function (hash) {
            self.postMessage({ id: d.id, hash: hash }, [hash.buffer]);
        })
        .catch(function (err) {
